...
class Ingestor:
    ...
    def ingest_text(self, text: str, source: str = "text") -> VectorStore:
        # Create a document object for the raw text
        documents = self.recursive_splitter.split_documents(
            self.semantic_splitter.create_documents([text])
        )

        # Embed the text and store it in the vector store
        return Qdrant.from_documents(
            documents=documents,
            embedding=self.embeddings,
            path=Config.Path.DATABASE_DIR,
